...
Eng duplicate
The text below is generated by a neural network because I'm too lazy to write it by hand in md format. After building and running the orchestrator, nothing will appear on the main page of the local host except a 404, so you need to go to /docs. On the /docs page, everything necessary will be available. The project also has a dashboard. It runs separately via npm on port 3000. The backend must be running before starting the frontend, otherwise the Dashboard will not be able to fetch data. IMPORTANT - run these two marvels from different terminals.

To start the dashboard: cd C:petdashboard npm run dev
